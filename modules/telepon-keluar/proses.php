...
// jika user sudah login, maka jalankan perintah untuk insert, update, dan delete
    ...
    if ($_GET['act']=='insert') {
        if (isset($_POST['simpan'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO sb_telepon_keluar(id_keluar,tanggal_keluar,ref_ticket,kode_telepon,jumlah,penerima,created_user) 
                                            VALUES('$id_keluar','$tanggal_keluar','$ref_ticket','$kode_telepon','$jumlah','$penerima','$created_user')")
                                            or die('Ada kesalahan pada query insert : '.mysqli_error($mysqli));    

            // cek query
            if ($query) {
                // perintah query untuk mengubah data pada tabel stok
                $query1 = mysqli_query($mysqli, "UPDATE sb_stok_telepon SET stok        = '$total_stok'
                ...
                if ($query1) {                       
                    ...
                    header("location: ../../main.php?module=telepon_keluar&alert=1");
                }
            }   
        }   
    }
