...
// jika user sudah login, maka jalankan perintah untuk insert dan update
	...
	if ($_GET['act']=='insert') {
		if (isset($_POST['simpan'])) {
            ...
            $query = mysqli_query($mysqli, "INSERT INTO sb_users(username,password,nama_user,hak_akses)
		...
		if (isset($_POST['simpan'])) {
			if (isset($_POST['id_user'])) {
				...
				if (empty($_POST['password']) && empty($_FILES['foto']['name'])) {
                    ...
                    $query = mysqli_query($mysqli, "UPDATE sb_users SET username 	= '$username',
                    													nama_user 	= '$nama_user',
                    													email       = '$email',
                    													telepon     = '$telepon',
                    													hak_akses   = '$hak_akses'
	...
	elseif ($_GET['act']=='on') {
		if (isset($_GET['id'])) {
			...
			// perintah query untuk mengubah data pada tabel users
            $query = mysqli_query($mysqli, "UPDATE sb_users SET status  = '$status'
                                                          WHERE id_user = '$id_user'")
