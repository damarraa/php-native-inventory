<script type="text/javascript">
  function tampil_barang(input){
    ...
    $.post("modules/stok-keluar/stok.php", {
      dataidbarang: num,
    }, function(response) {      
      $('#stok').html(response)
  ...
  function cek_jumlah(input) {
    ...
    if (jumlah > stok_tersedia) {
      ...
      window.location.href='main.php?module=form_stok_keluar&form=add';
    } else {
  ...
  function hitung_total_stok() {
    ...
    // tstok = document.formBarangKeluar.total_stok.value;
    // var jmlstok = eval(tstok);
    // if (tstok > stok_tersedia) {
    //   alert('Jumlah stok yang akan dikeluarkan melebihi stok yang tersedia.');
...
if ($_GET['form']=='add') { ?> 
  ...
  <section class="content-header">
    ...
    <ol class="breadcrumb">
      ...
      <li><a href="?module=stok_keluar"> Stock Barang </a></li>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="box box-primary">
          <!-- form start -->
          <form role="form" class="form-horizontal" action="modules/stok-keluar/proses.php?act=insert" method="POST" name="formBarangKeluar">
            <div class="box-body">
              <?php  
              // fungsi untuk membuat kode transaksi
              $query_id = mysqli_query($mysqli, "SELECT RIGHT(id_keluar,3) as kode FROM sb_barang_keluar
              ...
              <div class="form-group">
                ...
                <div class="col-sm-5">
                  <select class="chosen-select" name="kode_barang" data-placeholder="-- Pilih Barang --" onchange="tampil_barang(this)" autocomplete="off" required>
                    <option value=""></option>
                    <?php
                      $query_stok = mysqli_query($mysqli, "SELECT kode_barang, nama_barang FROM sb_stok_barang ORDER BY nama_barang ASC")
                                                            or die('Ada kesalahan pada query tampil stok: '.mysqli_error($mysqli));
                      while ($data_stok = mysqli_fetch_assoc($query_stok)) {
                        echo"<option value=\"$data_stok[kode_barang]\"> $data_stok[kode_barang] | $data_stok[nama_barang] </option>";
                      }
                    ?>
                  </select>
                </div>
            ...
            <div class="box-footer">
              <div class="form-group">
                <div class="col-sm-offset-2 col-sm-10">
                  ...
                  <a href="?module=stok_keluar" class="btn btn-default btn-reset">Batal</a>
                </div>
              </div>
            </div><!-- /.box footer -->
          </form>
